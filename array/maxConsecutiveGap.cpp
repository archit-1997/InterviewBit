int Solution::maximumGap(const vector<int> &A) {
    ...
    //suppose the interval is 2.33, this will yield as 2, but answer will be atleast three, so taking the greatest
    //integer
    ...
    //number of buckets is taken n-1 becuase we are following pigeon hole principle here
    //number of buckets is always equal to the number of intervals and not interval size

    vector<int> smallest(num_interval,INT_MAX),largest(num_interval,-1);
    //while inserting the elements into the bucket, we will not insert the min and max elements into any of the buckets
    //becuase this will effect our largest and smallest values at the first and last index although it's of no use
    //assigning each element into its bucket
    for(int i=0;i<n;i++){
