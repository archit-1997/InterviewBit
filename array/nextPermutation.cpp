...
vector<int> Solution::nextPermutation(vector<int> &A) {
  ...
  // from the right side, search for an element which is smaller than the
  // element on it's right side
  int right;
  for (int i = n - 2; i >= 0; i--) {
    if (A[i] < A[i + 1]) {
      right = i;
  ...
  int left = right + 1;
  // search from left to right for the next greater element greater than the
  // element on index right
  int small = A[left];
