...
/* -- if(arr[i]>arr[i+1]) in a rotated and sorted array, then arr[i] is the pivot element */

#include <bits/stdc++.h>
using namespace std;

//this function will return the index of the pivot element
//if the array is totally sorted and there is no pivot element, then this will return -1
int findPivot(vector<int> v,int low,int high){
    ...
    //if we have not found the pivot element yet, then recursively call for the two subarrays accordingly
    if(v[low]<=v[mid])
